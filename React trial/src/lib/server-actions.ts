...
export async function getFactoriesAction(): Promise<Option[]> {
  try {
    console.log('[getFactoriesAction] Starting...');
    ...
    console.log(`[getFactoriesAction] Found ${rows.length} factories`);
    const result = rows.map(row => ({
      label: row.plant_name || `Plant ${row.id}`,
      value: row.id.toString()
    }));
    console.log(`[getFactoriesAction] Returning ${result.length} options`);
    return result;
  } catch (error) {
    console.error('[getFactoriesAction] Error:', error);
    return [];
  }
}

export async function getFactorySectionsAction(factoryId: number): Promise<Option[]> {
  try {
...
export async function getItemsAction(factoryId: number, sectionId: string): Promise<Option[]> {
  try {
...
export async function getOperationsAction(factoryId: number, itemCode: string, sectionId: string): Promise<Option[]> {
  try {
    const buildingRows = await query<{ id: number }>(
      `SELECT id FROM master_buildingsectionlab WHERE building_id = $1 AND plant_id = $2 AND is_active = true`,
      [sectionId, factoryId]
...
export async function getParametersAction(factoryId: number, itemCode: string, operationId: string): Promise<Option[]> {
  try {
...
export async function getPurchaseOrdersAction(factoryId: number, itemCode?: string): Promise<Option[]> {
  try {
    let queryText = '';
    let params: any[] = [];
...
export async function getPurchaseOrderStatusAction(poId: string) {
  try {
...
export async function getFilteredInspectionsAction(filters: {
  factoryId: number;
  section: string;
  itemCode: string;
  ...
  try {
    ...
    if (type === 'In-process') {
      ...
      for (const row of rows) {
        const actualReadings = await query<{ r_value: number; r_key: string }>(
...
export async function getParameterSeriesAndStatsAction(
  factoryId: number,
  itemCode: string,
  operation: string,
  parameter: string,
...
export async function getLSLUSLDistributionAction(
  factoryId: number,
  itemCode: string,
  operation: string,
  parameter: string,
...
export async function getParameterDistributionAction(
